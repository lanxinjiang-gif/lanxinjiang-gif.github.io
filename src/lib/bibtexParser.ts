...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    // Parse year and month (undefined if not specified)
    const year = tags.year ? (parseInt(tags.year) || undefined) : undefined;
    ...
    // Parse order field (for explicit ordering)
    const order = tags.order !== undefined ? parseInt(tags.order) : undefined;

    // Parse nobibtex field — suppress the BibTeX button when true
    const nobibtex = tags.nobibtex === 'true' || tags.nobibtex === 'yes';
    ...
    const publication: Publication = {
      ...
      // Store original BibTeX (excluding custom fields), unless nobibtex is set
      bibtex: nobibtex ? undefined : reconstructBibTeX(entry, ['selected', 'preview', 'description', 'keywords', 'code', 'nobibtex', 'order']),
    };
    ...
    // Explicit order field takes top priority (lower number = first)
    const aOrder = a.order !== undefined ? a.order : Infinity;
    const bOrder = b.order !== undefined ? b.order : Infinity;
    if (aOrder !== bOrder) return aOrder - bOrder;

    // Publications with figures come first
    const aHasPreview = a.preview ? 1 : 0;
    const bHasPreview = b.preview ? 1 : 0;
    if (bHasPreview !== aHasPreview) return bHasPreview - aHasPreview;

    // Then sort by year (descending; entries without a year go last)
